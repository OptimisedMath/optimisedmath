...
@pytest.mark.parametrize(
    ...
    [
        (2, 2, 1),  # ahead of unlock frontier — no level unlock
        (2, 1, 2),  # behind unlock frontier — no level unlock
    ],
)
def test_unlock_requires_playing_at_unlock_frontier(streak, selected_level, unlocked_level):
    outcome = apply_turn(
        {"is_correct": True, "lock_answer": True},
        _ctx(
            streak=streak,
