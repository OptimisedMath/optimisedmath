...
def test_chapter_max_frontier_uses_last_topic_and_its_max_level():
    ...
    frontier = chapter_max_frontier(chapter_topics)

    assert frontier == Frontier(frontier_topic_id=30, frontier_level=3)


def test_chapter_max_frontier_defaults_when_no_topics():
    assert chapter_max_frontier([]) == Frontier(frontier_topic_id=1, frontier_level=1)
