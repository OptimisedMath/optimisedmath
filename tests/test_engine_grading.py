...
class TestMultipleChoiceGrading:
    def test_correct_answer(self):
        result = evaluate_answer("1", _sample_problem(), is_text_mode=False)
        assert result["is_correct"] is True
        assert result["lock_answer"] is True

    def test_trap_answer(self):
        result = evaluate_answer(
            r"\frac{2}{4}", _sample_problem(), is_text_mode=False
        )
    ...
    def test_wrong_answer(self):
        result = evaluate_answer("2", _sample_problem(), is_text_mode=False)
...
class TestTextGrading:
    def test_exact_text_match(self):
        result = evaluate_answer("1", _sample_problem(), is_text_mode=True)
        assert result["is_correct"] is True

    def test_syntax_error(self):
        result = evaluate_answer("abc", _sample_problem(), is_text_mode=True)
    ...
    def test_text_trap_match(self):
        ...
        result = evaluate_answer("2", problem, is_text_mode=True)
        assert result["trap_id"] == "w1"

    def test_missing_options_map_does_not_crash(self):
        problem = _sample_problem()
        del problem["options_map"]
        result = evaluate_answer("9", problem, is_text_mode=True)
        assert result["trap_id"] == "w1"
