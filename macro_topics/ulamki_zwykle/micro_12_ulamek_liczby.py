...
def frac_of_int_natural(level):
    while True:
        ...
        t1, _, _ = format_answers(k * d + n, d) # Trap 1: Added
        t2, _, _ = format_answers(k // n * d, 1) # Trap 2: Backwards logic
...
def frac_of_int_frac(level):
    while True:
        ...
        t1, _, _ = format_answers(k * d, n) # Trap 1: Divided
        t2, _, _ = format_answers(n * k, d * k) # Trap 2: Multiplied both
