...
def frac_ord_1():
    ...
    if template == "add_mul":
        a, b, c = [Fraction(random.randint(1, 3), random.choice(dens)) for _ in range(3)]
        ...
        b, c = [Fraction(random.randint(1, 3), random.choice([2,3,4])) for _ in range(2)]
        a = (b * c) + Fraction(random.randint(1, 2), random.choice([2,3])) 
        q = f"\\frac{{{a.numerator}}}{{{a.denominator}}} - \\frac{{{b.numerator}}}{{{b.denominator}}} \\cdot \\frac{{{c.numerator}}}{{{c.denominator}}}"
        ans = a - (b * c)
        t1, t2, t3 = (a - b) * c, a - b - c, a - b + c
        
    return build_problem_dict(q, f"{ans.numerator}/{ans.denominator}", 
