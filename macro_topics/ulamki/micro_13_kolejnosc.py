...
def order_no_brackets(level):
    while True:
        ...
        t1, _, _ = format_answers((n1 + n2) * n3, d * d) # Trap 1: Left to right
        t2, _, _ = format_answers(n1 + (n2 * n3), d) # Trap 2: Forgot common den
...
def order_brackets(level):
    while True:
        ...
        t1, _, _ = format_answers((n1 * n2) + n3, d * d) # Trap 1: Ignored brackets
        t2, _, _ = format_answers(n1 * (n2 + n3), d) # Trap 2: Multiplied straight across without proper den conversion
