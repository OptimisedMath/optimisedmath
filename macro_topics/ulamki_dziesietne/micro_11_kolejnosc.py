...
def dec_order_1():
    ...
    if template == "add_mul":
        ...
        a = round(ans_div * c, 2)
        b = round(random.randint(1, int(ans_div*10)-1) * 0.1, 1)
        q = f"{fmt_dec(a)} : {fmt_dec(c)} - {fmt_dec(b)}"
        ans = ans_div - b
        t1, t2, t3 = a / (c - b) if c != b else a, ans_div + b, a - c - b
        
    return build_problem_dict(q, fmt_dec(ans), t1=fmt_dec(t1), t2=fmt_dec(t2), t3=fmt_dec(t3))
...
def dec_order_2():
    ...
    if template == "brack_mul":
        ...
        c = round(random.randint(2, 5) * 0.1, 1)
        ans_div = round(random.randint(2, 6) * 0.1, 1)
        ab_sum = round(ans_div * c, 2)
        a = round(random.randint(1, int(ab_sum*10)-1) * 0.1, 1)
        b = round(ab_sum - a, 2)
        q = f"({fmt_dec(a)} + {fmt_dec(b)}) : {fmt_dec(c)}"
        ans = ans_div
        t1, t2, t3 = a + (b / c), (a + b) * c, a + b - c
