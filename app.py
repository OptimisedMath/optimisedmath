...
class StateManager:
    ...
    def init_defaults(macro_topics, curriculum):
        ...
        # Ensure progress dictionary catches new macro topics
...
with st.sidebar:
    ...
    admin_mode = st.toggle("🛠️ Tryb Admina", value=False)
...
if problem is None or "error" in problem:
    ...
    # Scaffolding: Input Mode Transition Check
    ...
    is_text_mode = (st.session_state.current_input_mode == "text")
    ...
    with st.form("answer_form", border=False):
        if is_text_mode:
            ...
            user_input = st.text_input("Twoja odpowiedź:", key=f"text_input_{st.session_state.last_id}")
        else:
            st.markdown("<style>.stRadio label { padding-bottom: 25px; padding-top: 10px; }</style>", unsafe_allow_html=True)
            render_option = lambda opt: f"${opt}$" if "\\" in opt else opt
            user_input = st.radio("Wybierz wynik:", st.session_state.shuffled_options, index=None, format_func=render_option)
    ...
    if admin_solve or submitted:
        if admin_solve:
            ...
            if user_input is None or str(user_input).strip() == "":
                st.warning("Najpierw podaj odpowiedź!")
                st.stop()
        ...
        if is_correct:
            ...
            if st.session_state.feedback_type != "info": # Soft errors don't break streaks
...
if st.session_state.problem_answered or st.session_state.get("feedback_type") in ["info", "warning"]:
    ...
    if fb_type == "success":
        ...
        if st.session_state.show_balloons == "level":
            st.balloons()
        ...
        elif st.session_state.show_balloons == "topic":
            st.balloons()
    ...
    if st.button(button_label, key="next_problem_btn"):
        if st.session_state.topic_completed:
            ...
            def is_duplicate(old, new):
                ...
                return old.get("question") == new.get("question") and old.get("correct") == new.get("correct")
