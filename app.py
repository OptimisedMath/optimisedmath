...
def full_hard_reset():
    ...
    st.session_state.progress = {
        mt: {
            "unlocked_order": curriculum[mt][0]["Topic_Order"] if curriculum[mt] else 1,
            "unlocked_level": 1,
        }
        for mt in macro_topics
    }
    st.session_state.selected_macro = macro_topics[0]
    st.session_state.selected_topic_order = (
        curriculum[macro_topics[0]][0]["Topic_Order"]
        if curriculum[macro_topics[0]]
        else 1
    )
...
for mt in macro_topics:
    ...
    if (
        mt not in st.session_state.progress
        or st.session_state.progress[mt]["unlocked_order"] < first_order
    ):
...
if (
    st.session_state.selected_topic_order is None
    or st.session_state.selected_topic_order < first_curr
):
...
topic_map = {
    row["Topic_Order"]: {"name": row["Micro_Topic"], "max_level": row["Level"]}
...
with st.sidebar:
    ...
    new_macro = st.selectbox(
        ...
        macro_topics,
        index=(
            macro_topics.index(st.session_state.selected_macro)
            if st.session_state.selected_macro in macro_topics
            else 0
        ),
    )
    ...
    if admin_mode:
        ...
        available_orders = [
            order for order in topic_map.keys() if order <= unlocked_order
        ]
    ...
    safe_index = (
        available_orders.index(st.session_state.selected_topic_order)
        if st.session_state.selected_topic_order in available_orders
        else len(available_orders) - 1
    )
    ...
    if (
        new_topic_order is not None
        and new_topic_order != st.session_state.selected_topic_order
    ):
    ...
    current_topic_max_level = topic_map.get(safe_order, {"max_level": 1}).get(
        "max_level", 1
    )
    ...
    if allowed_max_level == 1 and current_topic_max_level > 1:
        ...
        new_level = st.slider(
            "Wybierz Poziom:",
            min_value=1,
            max_value=allowed_max_level,
            value=safe_selected_level,
        )
...
if problem is None or "error" in problem:
    st.error(
        f"❌ Nie można załadować Poziomu {st.session_state.selected_level}. Sprawdź plik CSV."
    )
else:
    st.write("---")
    st.info(
        f"📍 {problem.get('level_display', 'Level ' + str(st.session_state.selected_level))}"
    )
    ...
    if (
        "shuffled_options" not in st.session_state
        or st.session_state.get("last_id") != problem["problem_id"]
    ):
    ...
    with st.form("answer_form", border=False):
        if st.session_state.current_input_mode == "radio":
            st.markdown(
                "<style>.stRadio label { padding-bottom: 25px; padding-top: 10px; }</style>",
            ...
            choice = st.radio(
                ...
                index=None,
                format_func=render_ui_option,
            )
            is_text_mode = False
        else:
            st.info("Wpisz wynik samodzielnie bez podpowiedzi.")
            st.markdown(
                "*(Format wprowadzania: wpisz **3/4**, a dla liczb mieszanych **1 1/2**)*"
            )
            user_text = st.text_input(
                "Twoja odpowiedź:", key=f"text_input_{st.session_state.last_id}"
            )
            is_text_mode = True
        ...
        with cols[0]:
            submitted = st.form_submit_button(
                "Sprawdź odpowiedź", disabled=st.session_state.problem_answered
            )
        with cols[1]:
            admin_solve = st.form_submit_button(
                "🪄 Auto-Solve", disabled=st.session_state.problem_answered
            )
...
if admin_solve or submitted:
    ...
    if is_correct:
        ...
        if (
            ...
            and st.session_state.selected_level == prog["unlocked_level"]
        ):
            current_topic_max = topic_map[st.session_state.selected_topic_order][
                "max_level"
            ]
...
if st.session_state.problem_answered or st.session_state.get("feedback_type") in [
    "info",
    "warning",
]:
    if st.session_state.get("feedback_type") == "success":
        ...
        if st.session_state.get("show_balloons") == "level":
            ...
            prog = st.session_state.progress[st.session_state.selected_macro]
            st.success(
                f"🎊 Poziom {prog['unlocked_level']} odblokowany! Zostałeś automatycznie przeniesiony na nowy poziom. 🎊"
            )
            st.session_state.show_balloons = False
        elif st.session_state.get("show_balloons") == "topic":
            st.balloons()
            st.success(
                "🏆 Gratulacje! Ukończyłeś cały temat. Jesteś gotowy, aby przejść do kolejnego!"
            )
            st.session_state.show_balloons = False
...
if st.session_state.problem_answered:
    ...
    if st.button(button_label, key="next_problem_btn"):
        ...
        if st.session_state.get("topic_completed"):
            next_topics = [
                t for t in topic_map.keys() if t > st.session_state.selected_topic_order
            ]
            if next_topics:
                next_topic_id = min(next_topics)
                st.session_state.progress[st.session_state.selected_macro][
                    "unlocked_order"
                ] = next_topic_id
                st.session_state.progress[st.session_state.selected_macro][
                    "unlocked_level"
                ...
                st.success(
                    "🎉 Gratulacje! Ukończyłeś cały ten dział. Wybierz nowy dział z menu po lewej stronie."
                )
        ...
        # --- NORMAL NEXT PROBLEM LOGIC ---
            ...
            if (
                st.session_state.streak >= 1
                and "Porównywanie"
                not in topic_map[st.session_state.selected_topic_order]["name"]
            ):
