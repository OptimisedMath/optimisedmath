...
def frac_add_1() -> dict | None:
    ...
    t1, _ = format_answers(n1 + n2, d + d)
    w1, _ = format_answers(n1 + n2 + 1, d)
    w2, _ = format_answers(abs(n1 + n2 - 1), d)
...
def frac_add_4() -> dict | None:
    ...
    t1, _ = format_answers(n1 + n2, d + d, w1 + w2)
    w1_str, _ = format_answers((w1 * d + n1) + (w2 * d + n2) + d, d)
    w2_str, _ = format_answers((w1 * d + n1) + (w2 * d + n2) + 1, d)
...
def frac_add_5() -> dict | None:
    ...
    c_str, _ = format_answers((num1 * d2) + (num2 * d1), d1 * d2)
    t1, _ = format_answers(n1 + n2, d1 * d2, w1 + w2)
    t2, _ = format_answers(n1 + n2, d1 + d2, w1 + w2)
    w1_str, _ = format_answers((num1 * d2) + (num2 * d1) + 1, d1 * d2)
