"""Figure constructors: maths in, geometry out.
...
def unit(p: Pt) -> Pt:
    """Vector `p` scaled to length 1; the zero vector is returned unchanged."""
    n = norm(p)
...
def _signed_area(pts: list[Pt]) -> float:
    """Twice the signed area of a closed walk — positive when it runs counter-clockwise."""
    s = 0.0
...
class Figure:
    ...
    def edge(self, e: str) -> tuple[Pt, Pt]:
        """The two endpoints of edge `e`, named by its two vertices (`"AB"`)."""
        return self.points[e[0]], self.points[e[1]]

    def edge_length(self, e: str) -> float:
        """Length of edge `e`, measured on the constructed figure."""
        a, b = self.edge(e)
        return norm(sub(b, a))

    def neighbours(self, v: str) -> tuple[str, str]:
        """The vertices either side of `v` along the outline."""
...
class Triangle:
    """Triangle constructors, one per set of measurements a Topic states."""
...
def rectangle(width: float, height: float) -> Figure:
    """Axis-aligned rectangle with A at the origin."""
    return polygon(
...
def square(side: float) -> Figure:
    """Axis-aligned square with A at the origin."""
...
def rhombus(side: float, angle: float = 60.0) -> Figure:
    """Rhombus from a side and the angle at A."""
...
def circle(radius: float) -> Figure:
    """A circle — the one figure with no vertices, so annotations name its centre."""
    return Figure(
