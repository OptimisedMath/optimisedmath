...
# Columns `CREATE TABLE` declares that `_TELEMETRY_COLUMNS` doesn't — generated by
# SQLite itself, never part of the INSERT, but still part of the shape a
# pre-existing table has to match.
_TELEMETRY_GENERATED_COLUMNS = frozenset({"log_id", "timestamp"})
...
def _drop_stale_telemetry_table(cursor: sqlite3.Cursor) -> None:
    """Drop telemetry_logs if its columns don't match `_TELEMETRY_COLUMNS` exactly.

    Pre-existing telemetry rows are dropped, not migrated, when the schema changes
    shape — adding, renaming or removing a column in `_TELEMETRY_COLUMNS` is what
    makes that happen. An exact match, not a subset check, so a column that
    stops being written (like `is_correct` in #253) also forces a drop, rather
    than being left behind as dead NOT NULL state a future INSERT can't satisfy.
    Acceptable pre-launch, while telemetry has no production readers yet; past
    ...
    expected_columns = set(_TELEMETRY_COLUMNS) | _TELEMETRY_GENERATED_COLUMNS
