...
6. **Chapter-end fallback:** when Topic completion leaves no next Topic, or the next Topic is Locked, return the already-completed Problem rather than regenerating.
7. **Post-Topic-completion auto-Navigation targets the next Topic after the Selected one, not a Frontier.** It reads no Frontier field; the existing validate-and-resolve path still decides whether that Topic is Reachable, so a Locked target cannot be landed on.
