...
4. **`persists_profile = False` is the only thing that stops the profile write.** Every Submission still runs the normal grade → progression → respond pipeline and telemetry still logs; the write-back restores XP and the Chapter Frontiers from the stored profile, so neither the in-cycle XP nor the moved Frontier ever reaches it. Streak and Flawless have no profile column at all ([ADR-0006](../../docs/adr/0006-profile-owns-selected-not-streak-or-flawless.md)) and are written from the Session into the stored Session in both play modes, exactly like a Student's — the in-cycle Streak runs for a realistic feel and there is nothing to restore it from.
