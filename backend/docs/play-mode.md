...
4. **`persists_profile = False` is the only thing that stops the write.** Every Submission still runs the normal grade → progression → respond pipeline and telemetry still logs, labelled `play_mode = 'admin'` rather than discarded; XP, Flawless, and Frontier are simply never written back. Session streak still runs in-cycle for a realistic feel, and is never persisted.
