...
4. **The trigger and the Deconstruction's own ending must agree on one identity: the Misconception slug.** The trigger's count and guard, and `deconstruction_step._disarm()`, all key on `misconception_slug` alone — no Chapter, Topic or Level — and `_disarm()` reads it off the active Deconstruction's own state, never off the Session's Selected chapter, topic or level ([ADR-0014](../../docs/adr/0014-deconstruction-trigger-counts-per-session.md)).
