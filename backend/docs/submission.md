...
4. **The trigger and the Deconstruction's own ending must agree on one identity.** The trigger check and `state.misconception_hits` share their `(Misconception, Level)` key format with `deconstruction_step.deconstruction_key()`; changing one changes all three.
