...
- **API contract/calls** — request/response shapes are `backend/models.py` (Pydantic) mirrored by `lib/session/types.ts` and `lib/types.ts`; relative `/api/*` paths, Next.js rewrites to FastAPI on `:8000`.
