...
2. **`SessionClient` is the app's one test seam.** It expresses the operations performed on a Session — start, submit, next problem, navigate, reset, end — in domain terms. There are exactly two adapters: `httpSessionClient` (wired in `app/layout.tsx`) and the in-memory adapter in `test/fakeBackend.ts`.
...
5. **The Deconstruction takeover computes no outcomes** ([ADR-0002](../../docs/adr/0002-backend-owns-game-rules.md)). `useDeconstruction()` owns only the local phase machine — `pause -> intro -> step (looping) -> handback`, entered at `intro` rather than `pause` when the first Session of a page load already has one running, because that Resume's pause happened before the load ([ADR-0019](../../docs/adr/0019-refresh-resumes-the-session.md)) — plus Abandonment. The trigger is `SessionResponse.deconstruction_running`; step correctness is `DeconstructionSubmissionResponse.is_correct`.
